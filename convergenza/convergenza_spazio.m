%%% Convergenza nello spazio del modello 1D parete
...
flux=q_par_u*rapp_aree/coeff_mitigazione;

%% Discretizzazione nel tempo
dt=0.1;

%% Convergenza nello spazio
dxv=[5e-7,1e-6,2e-6,5e-6,1e-5,2e-5];
...
for mm=1:length(dxv)
    ...
    if mm==1
        ...
        err_num(mm)=norm(TT-TTref(1:round(dx/dx_ref):end))/norm(TTref(1:round(dx/dx_ref):end)-Tw);
        RA(mm)=NCOND/(1-eps*NCOND)*2*eps;
    end
end

%% grafici
figure(1)
loglog(dxv,err_num,'b-o')
grid minor
hold on
loglog(dxv,1e-3*ones(size(dxv)),'k--')
loglog(1e-5,err_num(end-1),'o','markerfacecolor','g','markersize',10)
loglog(dxv,RA,'r-o')
xlabel('\Deltax[m]')
ylabel('Errore relativo [-]')
xlim([dxv(2) dxv(end)])
title('Errore rispetto alla soluzione più raffinata')
legend('Errore numerico','Tolleranza richiesta','\Deltax scelto','\epsilon_R_0','location','sw')
